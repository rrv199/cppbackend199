...
void RequestHandler::operator()(StringRequest&& req, std::function<void(StringResponse&&)> send) {
    ...
    try {
        ...
        // Отладка
        std::cerr << "DEBUG: static_root = " << static_root_.string() << std::endl;
        std::cerr << "DEBUG: target = " << target << std::endl;
        std::cerr << "DEBUG: decoded = " << decoded << std::endl;
        std::cerr << "DEBUG: file_path = " << file_path.string() << std::endl;
        std::cerr << "DEBUG: canonical_path = " << canonical_path.string() << std::endl;
        
        // Проверка безопасности
        if (canonical_path.string().find(static_root_.string()) != 0) {
        ...
        if (!fs::exists(canonical_path) || !fs::is_regular_file(canonical_path)) {
            std::cerr << "DEBUG: File not found: " << canonical_path.string() << std::endl;
