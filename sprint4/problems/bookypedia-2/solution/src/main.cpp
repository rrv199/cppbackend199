    ...
    try {
        ...
        while (getline(cin, line)) {
            ...
            if (cmd == "AddAuthor") {
                ...
                try {
                    pqxx::work w(conn);
                    w.exec_params("DELETE FROM authors WHERE name = $1", name);
                    w.commit();
                } catch (const exception& e) {
                    cout << "Failed to delete author" << endl;
                }
                ...
                try {
                    pqxx::work w(conn);
                    w.exec_params("UPDATE authors SET name = $1 WHERE name = $2",
                                  new_name, name);
                    w.commit();
                } catch (const exception& e) {
                    cout << "Failed to edit author" << endl;
                ...
                start = author_name.find_first_not_of(" \t");
                if (start != string::npos) {
                    author_name = author_name.substr(start);
                }
                end = author_name.find_last_not_of(" \t");
                if (end != string::npos) {
                    author_name = author_name.substr(0, end + 1);
                }
                
                string author_id;
                if (!author_name.empty()) {
                    pqxx::read_transaction r(conn);
                    auto res = r.exec_params("SELECT id FROM authors WHERE name = $1", author_name);
                    if (res.empty()) {
