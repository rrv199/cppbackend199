    ...
    try {
        ...
        while (getline(cin, line)) {
            ...
            if (cmd == "AddAuthor") {
                ...
                try {
                    pqxx::connection conn(db_url);
                    auto res = conn.exec("SELECT name FROM authors ORDER BY name");
                    int i = 1;
                ...
                try {
                    pqxx::connection conn(db_url);
                    auto res = conn.exec("SELECT b.title, a.name, b.publication_year FROM books b JOIN authors a ON b.author_id = a.id ORDER BY b.title, a.name, b.publication_year");
                    int i = 1;
                    for (const auto& row : res) {
                ...
                try {
                    ...
                    if (res.size() == 1) {
                        ...
                        auto tag_res = conn.exec_params("SELECT tag FROM book_tags WHERE book_id = $1 ORDER BY tag", row[0].as<string>());
                        if (!tag_res.empty()) {
                            cout << "Tags: ";
                    ...
                    const auto& row = res[choice - 1];
                    cout << "Title: " << row[1].as<string>() << endl;
                    cout << "Author: " << row[2].as<string>() << endl;
                    cout << "Publication year: " << row[3].as<int>() << endl;
                    
                    auto tag_res = conn.exec_params("SELECT tag FROM book_tags WHERE book_id = $1 ORDER BY tag", book_ids[choice - 1]);
                    if (!tag_res.empty()) {
                        cout << "Tags: ";
                        for (size_t j = 0; j < tag_res.size(); ++j) {
                            if (j > 0) cout << ", ";
                ...
                try {
                    ...
                    if (!title.empty()) {
                        ...
                        if (res.size() == 1) {
                            ...
                            auto full_res = conn.exec_params(
                                "SELECT b.id, b.title, a.name, b.publication_year "
                                "FROM books b JOIN authors a ON b.author_id = a.id "
                ...
                try {
                    ...
                    if (res.size() == 1 && !title.empty()) {
                        ...
                        for (size_t i = 0; i < all_books.size(); ++i) {
                            const auto& row = all_books[i];
                            auto author_res = conn.exec_params(
                    ...
                    auto year_res = conn.exec_params("SELECT publication_year FROM books WHERE id = $1", book_id);
                    ...
                    auto tag_res = conn.exec_params("SELECT tag FROM book_tags WHERE book_id = $1 ORDER BY tag", book_id);
                    string tags_str;
                ...
                if (!author_input.empty()) {
                    try {
                        pqxx::connection conn(db_url);
                        auto res = conn.exec_params("SELECT id FROM authors WHERE name = $1", author_input);
                        if (!res.empty()) {
                    ...
                    try {
                        pqxx::connection conn(db_url);
                        auto res = conn.exec("SELECT id, name FROM authors ORDER BY name");
