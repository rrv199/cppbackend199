...
class ItemCollector : public ItemGathererProvider {
public:
    struct GameItem {
        Point position;
        double width = 0.0;
    ...
    void AddItem(size_t id, size_t type, Point pos, size_t value) {
        items_.push_back({pos, 0.0, type, id, value, false});
    }
    ...
    Item GetItem(size_t idx) const override {
        return {items_[idx].position, items_[idx].width, items_[idx].value, items_[idx].type};
    }
    ...
    void ProcessTick() {
        ...
        for (const auto& event : events) {
            ...
            if (!item.is_collected) {
                if (gatherer.player->AddItem(item.id, item.type, item.value)) {
    ...
    const std::vector<GameItem>& GetItems() const { return items_; }
    const std::vector<GameGatherer>& GetGatherers() const { return gatherers_; }
