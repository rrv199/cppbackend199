...
public:
    ...
    std::pair<int, std::string> CreatePlayer(const std::string& name, const std::string& map_id) {
        ...
        players_[id] = std::make_shared<Player>(id, name, token, map_id);
        token_to_player_[token] = players_[id];
        
        // Добавляем в список игроков на карте
        map_players_[map_id][id] = players_[id];
