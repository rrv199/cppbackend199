...
StringResponse HandleJoinGame(const json::value& body) {
    try {
        ...
        auto& game = PlayerManager::Instance(); // Временно, нужно получить game
        ...
        return MakeStringResponse(http::status::ok, json::serialize(result), 11, true);
    ...
    if (target.find("api/") == 0) {
        ...
        if (target == "api/v1/game/join") {
            if (req.method() != http::verb::post) {
                ...
                auto result = HandleJoinGame(json::parse(req.body()));
                body = result.body();
